...
site %>% write_csv(here("data", "site.csv"))
...
## Create table describing each reintroduced cohort - for review of what cohorts to include in analysis

Information to include in table

-   Release site ID and basin name (avoid revealing location information

-   public repo)

-   Species (R. muscosa or R. sierrae)

-   Zoo

-   Source population type (naive, epizootic, persisting, recovering)

-   Bd load at end of zoo Bd exposure for exposed and unexposed frogs (median, range).

-   Bd isolates - Any Bd positives in unexposed group?
...
filter(!(bd_exposure == "")) %>% 
group_by(release_siteid1, zoo, release_date, bd_exposure) %>%
...
filter(exposed > 0, unexposed > 0) %>% # 12 reintroduced cohorts included only unexposed frogs
mutate(zoo = replace_na(zoo, "ucsb")) %>% 
arrange(release_siteid1, release_date)

# Add species column
exposed_unexposed <- exposed_unexposed %>% 
...
# add status of each source population
- create list of each reintroduced cohort and associated source populations
- assign status to each source population (naive, epizootic, persisting, recovering)
- add to table using pivot_wider followed by concatenate to create single column containing comma-separated values
...
-   Identify issues with all exposed-unexposed reintroductions by site-date (see notebook.qmd).

    -   \- Roland takes \< 2019, Tom takes \>= 2019

-   Zoo swabs from pre-reintroduction frogs: where are these data stored?
...
## Create dataset of reintroduced frogs and relevant characteristics - for analysis
